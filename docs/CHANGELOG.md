...
Проектът е изграден основно за около две седмици (13–26 юли 2026), на седем ясни етапа: (1) базова автоматизация на тренировъчния анализ, (2) PHP dashboard, (3) здравни алерти по ключови думи, (4) преработка на цялата alert система за надеждност, (5) лабораторни лактатни данни и анализ, (6) местни резултати + национални функционални тестове, (7) redesign на НЦ секцията и инфраструктурни корекции. Осми, по-разпънат във времето етап (12–17 август 2026) добави условия на състезанието (темп./wetsuit), данни за курса на състезанието (`race_courses`), преработено сравнение на стартове, поправка на пропуск в zone-данните и цялостен UI/UX одит на дашборда. Всеки етап е надграждал предишния, без да го чупи — виж git история за пълни детайли, тук е обобщено по тема, не commit по commit.
...
## Phase 8 — Race conditions, race courses, comparison redesign, a zone-data gap, and a UI/UX audit (2026-08-12 to 2026-08-17)

- **World Triathlon race-day conditions**: `fetch_world_triathlon.py`'s per-split-position event fetch was already pulling `meta` (temperature_water/temperature_air/humidity/wbgt/wind/weather/wetsuit/drafting) and discarding it — `parse_conditions()` now keeps it, `compute_missing_result_metadata()` (renamed from `compute_missing_split_positions()`) computes positions and conditions together in one pass keyed by `(event_id, prog_id)`, since both come from the same API call. New `conditions_computed_at` marker, decoupled from `positions_computed_at`, let existing rows backfill independently. Shown on `athlete.php` as small chips (🌊/🌡️/wetsuit) in the split panel.
- **`race_courses`** — coach-maintained course-layout data (swim/bike/run distances, laps, start type, surface, coach notes) synced from a new "Курсове" tab in the local-results Sheet via `fetch_race_courses.py`. `event_id` is the numeric World Triathlon event ID (not a `local_events`-style slug — see [spec_race_courses.md](spec_race_courses.md) §5), cross-checked against production `world_triathlon_results` rather than assumed; `event_title` is never typed into the Sheet, it auto-fills at sync time from `world_triathlon_results` by `event_id` lookup and doubles as an "did this ID actually resolve" signal. Rendered as a 3-column "Курс" section (Плуване/Вело/Бягане) in the same split panel as the conditions chips above. New `scripts/audit_data.py` check (#7) flags 2026 results missing a course row and course rows whose `event_id` doesn't resolve.
- **Combined-hronology redesign**: the flat, unfiltered "Обединена хронология" table on `athlete.php` (every local + WT result, no selection) was replaced with a compact card (mini bar-timeline, height ∝ total time, + Стартове/Последен/Най-бърз спринт stats) linking to a new dedicated page, `results_comparison.php` — year-filtered checkbox selection (persists across year switches and is bookmarkable via `?races=`), a metrics-as-rows/starts-as-columns comparison table with a Δ column (minutes:seconds, colored by whether the newest selected start was faster), and a "compare with another athlete" dropdown pooling both athletes' selectable starts into one table. New shared `includes/wt_event_meta.php`: a manually-curated, API-verified relay-exclusion list (`WT_RELAY_EVENT_PROG_IDS` — Mixed/Team/Youth Relay results share `event_id` with the athlete's individual result at the same competition and rendered as apparent duplicates) and a Sprint/Super-Sprint distance map, both static and requiring manual updates for future events not yet in either list.
- **`activity_zones` 7-day window gap**: "Зони на тренировка" was silently missing entire activity types (road `Ride`, pool `Swim`) for every athlete — root cause was `fetch_intervals.py:get_activities()`'s 7-day rolling window combined with athletes cycling/pool-swimming less often than weekly, so their (rare) qualifying activities fell between cron runs and were never fetched. Confirmed via the raw Intervals.icu API that some `Ride` activities had complete zone data 30-75 days old. Window widened to 30 days; a one-time manual backfill (`days=90`) closed the existing gap (Мира 11→90 rows, Симеон 14→70, Миролюба 1→5). Documented as a runbook incident since the failure mode (a whole activity *type* silently absent, not one missing row) isn't obvious from the symptom alone.
- **UI/UX audit + 5-item rollout** across all four dashboard pages (full findings: [audit artifact](https://claude.ai/code/artifact/496deeb2-3f88-4e79-857e-99e040d9512f)) — (1) new `includes/design_tokens.php` shares the `--ink`/`--ink-2`/`--muted`/`--grid`/`--surface` token set and `status_badge()` across all four pages instead of three copy-pasted `:root` blocks and a fourth page (`dashboard.php`, the post-login landing page) with none of them at all; (2) named `--action` (UI chrome) as distinct from `--series-1`/`--series-2` (data encoding only) and `--good`/`--warn`/`--bad` (the status triple that was already consistent everywhere but only existed as repeated hex literals); (3) a sticky anchor nav on `athlete.php` (Тренировка/Състезания/Зони/Лаборатория) — the page had grown to 11+ stacked sections with no in-page navigation; (4) consolidated `.pos-badge`/`.source-badge`/`.lt-badge`/`.eval-badge`/`.condition-chip` (five different border-radius values for the same "colored pill" job) into a shared `.pill` base via combined CSS selectors, zero template changes; (5) synchronized hover across the six training-load charts (ACWR/Load/HRV/Sleep/World/Regional) — World/Regional Ranking's sparser label set (`rankLabels`, ranking fetched 3x/week vs. the daily-metrics four) meant the sync had to match by date *value*, not raw index, or it would highlight the wrong day on the ranking charts.
