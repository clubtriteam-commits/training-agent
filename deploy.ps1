...
scp -P $port -r @items "${target}:${remotePath}/"
...
ssh -p $port $target "find '$remotePath' -type d -exec chmod 755 {} + && find '$remotePath' -type f -exec chmod 644 {} +"
if ($LASTEXITCODE -ne 0) { Write-Host "chmod се провали" -ForegroundColor Red; exit 1 }

# 4. Проверка: показваме какво има на сървъра
ssh -p $port $target "ls -la '$remotePath'"
